...
Or in a single step without touching your working directory:
...
On a TTY jip asks before cloning (`clone <url> into ./<repo-name>? [Y/n]`). A repository without a Maven/Gradle build is refused with an error and the directory is left in place; `./<repo-name>` already existing is an error, so check twice before deleting anything.
